...
class SecretAgent(CaptureAgent):
  ...
  def getTarget(self, gameState, enemyIndex):
    ...
    if self.distancer.getDistance(myPos, enemyPos) > 2:
      ...
      if enemyPos[0] == myPos[0] and enemyPos[1] == myPos[1] + 1:
        target = (enemyPos[0], enemyPos[1] + 2)
      elif enemyPos[0] == myPos[0] and enemyPos[1] == myPos[1] - 1:
        target = (enemyPos[0], enemyPos[1] - 2)
      elif enemyPos[0] - 1 == myPos[0] and enemyPos[1] == myPos[1]:
        target = (enemyPos[0] - 2, enemyPos[1])
      elif enemyPos[0] + 1 == myPos[0] and enemyPos[1] == myPos[1]:
        target = (enemyPos[0] + 2, enemyPos[1])
    return target
  ...
  def getFeatures(self, gameState):
    ...
    if self.strategies[self.index] == 'Attack':
      ...
      if gameState.hasWall(self.currentTarget[0], self.currentTarget[1]):
      ...
      features['timeLeft'] = gameState.data.timeleft
      ...
      features['hunterDist'] = min([self.getMazeDistance(myPos, l) for l in knownEnemyLocations])
  ...
  def getWeights(self, gameState):
    if self.strategies[self.index] == 'Attack':
      ...
      #print("chase")
      weights = {'targetDist': -5, 'timeLeft': -10000}
    elif self.strategies[self.index] == 'Scatter':
  ...
  def getScoreIfTerminal(self, gameState):
    ...
    if self.strategies[self.index] == 'Scatter':
      ...
      # Is no longer prey
      selfState = gameState.getAgentState(self.index)
      enemyState = gameState.getAgentState(self.seenEnemies[0])
  ...
  def evalStratChange(self, gameState, enemyIndex):
    ...
    if (enemyState.isPacman or ((distance / 2 < enemyState.scaredTimer) and 
            enemyState.scaredTimer > 3)):
      return 'Chase'
    elif ((selfState.scaredTimer > distance) or selfState.isPacman) \
              and distance < 4 and len(self.getFood(gameState).asList()) > 4:
      return 'Scatter'
    return 'Attack'
  ...
  def getGoodLegalActions(self, gameState, index):
    ...
    if self.strategies[index] == 'Chase' or self.strategies[index] == 'Scatter':
