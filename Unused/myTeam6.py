...
class SecretAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if (gameState.getAgentState(self.index).isPacman or 
        self.myPos == gameState.getInitialAgentPosition(self.index)):
      self.strategies[self.index] = "Attack"
    if not self.seenEnemies:
      if self.strategies[self.index] == "Chase" or self.strategies[self.index] == "Scatter":
        self.strategies[self.index] = "Attack"
    else:
      newStrat = None
    ...
    # Debugging strategy
    # print(self.index, self.strategies[self.index])
    ...
    print('MYTEAM6', time.clock() - self.startTime)
    return bestAction
  ...
  def evalStratChange(self, gameState, enemyIndex):
    ...
    if ((enemyState.isPacman and selfState.scaredTimer < distance / 2) 
        or (distance / 2 < enemyState.scaredTimer)) and distance < 6:
      return 'Chase'
    elif ((selfState.scaredTimer > distance) or selfState.isPacman) \
              and distance < 5 and len(self.getFood(gameState).asList()) > 4:
      return 'Scatter'
    elif not selfState.isPacman and not enemyState.isPacman and distance < 6:
